...
public class OrderController {
    ...
    public StockOrderDto createStockOrder(@RequestBody CreateStockOrderRequest request) {
        return orderService.createStockOrder(request.customerId(), request.carId());
    }
    ...
    public CustomOrderDto createCustomOrder(@RequestBody CreateCustomOrderRequest request) {
        return orderService.createCustomOrder(request.customerId(), request.carModelId(), request.selectedOptionIds());
    }
    ...
    public record CreateStockOrderRequest(UUID customerId, UUID carId) {
    }

    public record ChangeStockStatusRequest(StockOrderStatus status) {
    }

    public record CreateCustomOrderRequest(UUID customerId, UUID carModelId, Map<ConfigType, UUID> selectedOptionIds) {
    }
