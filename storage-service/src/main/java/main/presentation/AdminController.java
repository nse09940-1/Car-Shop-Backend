...
public class AdminController {
    ...
    @PostMapping("/cars")
    public void saveCar(@RequestBody CarSaveRequest request) {
        Car car = new Car(
                request.id() == null ? UUID.randomUUID() : request.id(),
                request.vin(),
                request.carModelId(),
                ...
                request.available(),
                request.availableForTestDrive());
        systemAdminService.saveCar(car);
    }

    @GetMapping("/cars")
    public List<CarCardDto> listCars() {
        return systemAdminService.listCars();
    }

    @DeleteMapping("/cars/{id}")
    public void deleteCar(@PathVariable UUID id) {
        systemAdminService.deleteCar(id);
    }

    @PostMapping("/parts")
    public void savePart(@RequestBody PartDto request) {
        CarPart part = new CarPart(
                ...
                request.inStock(),
                request.reserved());
        systemAdminService.savePart(part);
    }

    @GetMapping("/parts")
    public List<PartDto> listParts() {
        return systemAdminService.listParts();
    }

    @DeleteMapping("/parts/{id}")
    public void deletePart(@PathVariable UUID id) {
        systemAdminService.deletePart(id);
    }
